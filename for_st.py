...
if files:
    ...
    for_check['check'] = "<div class='blocks' style='width: 100%; height: 100%; display: flex; justify-content: center; align-items: left'><u>" + for_check['block'] + '</u> : <b>' + for_check['quantity'].astype(str) + " шт.</b></div>"
    pivot_check = for_check.pivot_table(index='description', values=['check'], aggfunc=lambda x: '<br>'.join(x)).reset_index()
    html_text = "<br>".join(["<div class='canal-installation'><div class='system' style='width: 100%; height: 100%; display: flex; justify-content: center; align-items: left'><b>" + pivot_check.iloc[i, 0] + "</div></b> :<br>" + pivot_check.iloc[i, 1] + "<br></div>" for i in range(pivot_check.shape[0])])
